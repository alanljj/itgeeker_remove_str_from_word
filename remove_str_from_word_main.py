...
class ListSheet(tk.Frame):
        ...
        if not val_list:
            ...
            if finished_nmb:
                messagebox.showinfo(title="任务通知", message="任务已圆满完成！共处理了%s个文件,\n"
                                                              "目录里旧版word .doc文件已转换成新的.docx的文件,\n"
                                                              "保存在源文件相同目录, 文件名带有-converted字样。"
                                                              % str(finished_nmb))
            else:
    ...
    def string_frame(self):
        ...
        self.entry_str = tk.Entry(string_frame, justify=tk.CENTER, width=60,
        ...
        self.entry_str.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky='nsew')
    ...
    def select_path_frame(self):
        ...
        self.entry_path = ttk.Entry(mnplt_frame, justify=tk.LEFT, width=60,
        ...
        self.entry_path.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky='nsew')
    ...
    def author_frame(self):
        ...
        label_ver = ttk.Label(author_frame, text='开源版本Ver 1.1.1.0', font=('Microsoft YaHei UI', 10), cursor="heart")
